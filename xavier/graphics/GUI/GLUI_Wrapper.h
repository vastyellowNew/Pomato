...
static vector<GLUI_Checkbox_Wrapper> checkboxes;
...
static vector<GLUI_Spinner_Wrapper> spinners;
...
static vector<GLUI_Listbox_Wrapper> listboxes;
...
static vector<GLUI_RadioButtonGroup_Wrapper> radioButtonGroups;
...
// since C++ only allows integer values used with #if, you need to edit glui.h 
// so that the original GLUI_VERSION is multiplied by 100
#if 1
...
static vector<GLUI_Scrollbar_Wrapper> scrollbars;
...
struct ScrollbarArray {
	vector<GLUI_Scrollbar*> scrollbars;
};
...
static vector<GLUI_Button_Wrapper> buttons;
