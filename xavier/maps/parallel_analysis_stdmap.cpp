...
// who would have thought that one would have to implement this?
...
static void init()
{
    ...
    // we allow for positive and negative winding factors
    for (int pol_r=-maxp ; pol_r<=maxp ; ++pol_r) {
        for (int tor_r=1 ; tor_r<=maxp ; ++tor_r) {
...
void __multi_cell_index(const nvis::ivec2& cell_id, int p,
                        ...
                        bool verbose=false)
{
    ...
    try {
        ...
        if (idx != 0) {
            ...
            std::cerr << os.str();
            idx_boxes.push_back(tagged<p_box_type>(p_box_type(bounds, p), idx));
            if (verbose) {
    ...
    #pragma omp parallel
    {
        ...
        for (int n=0 ; n<_boxes_indices.size() ; ++n) {
            ...
            if (found) {
                ...
                for (int i=1 ; i<chain.size() ; ++i) {
                    p_cell_type id(_plane->local_coordinates(chain[i]).first, period);
                    discovered[thread_id].push_back(solution_type(id, chain[i]));
                }
                std::ostringstream os;
                os << fp << " found" << std::endl;
                std::cerr << os.str();
...
void filter_chains()
{
    ...
    std::cerr << _fixpoints.size() << " fixed points in input of filter_chains()\n";
...
void check_cell(const nvis::ivec2& cell_id)
{
    ...
    _params.valid_rationals.insert(_valid_rationals.begin(), _valid_rationals.end());
    ...
    if (found) {
        ...
        int p = fp.K;
        std::vector<tagged<p_box_type> > tmp;
        int index = __index(cell_id, p, tmp, true);
...
void check_orbit(const nvis::vec2& x)
{
    ...
    double wn = pmap.winding_number(x, steps);
    std::cerr << "winding number at " << x << " is " << wn << '\n';
    
    boost::rational<int> q = rational_approx(wn, maxp);
    std::cerr << "best rational approximation within prescribed bounds is " << q << std::endl;
    
    std::cerr << "approximation error = " << fabs(wn-value<int, double>(q)) << std::endl;
...
void draw(void)
{
    ...
    if (show_vec) {
        ...
        for (int n=0 ; n<_dataset->size() ; ++n) {
            ...
            int p = best_period(d.steps, maxp, _plane_metric);
            nvis::vec2 v = p_vector(d, p, _plane_metric);
            if (nvis::norm(v) == 0) {
                continue;
            }
            nvis::vec2 p1 = p0 + v;
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    if(key == 'r') {
        ...
        map2d::return_type res = pmap.map_complete(_last, period);
        std::cerr << "1-jacobian at " << _last << " is " << res.J << std::endl;
        nvis::mat2& J = res.J;
        fixpoint fp;
        fp.pos = x;
...
int main(int argc, char** argv)
{
    ...
    if (!nogfx) {
        ...
        std::cerr << "after creating window\n";
        display_camera_info();
    }
