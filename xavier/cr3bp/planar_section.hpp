...
#ifndef __CR3BP_PLANAR_SECTION_HPP__
#define __CR3BP_PLANAR_SECTION_HPP__
...
class planar_section : public section<double, N2, 2> {
public:
    ...
    typedef planar_section<rhs_type, N1, N2> self_type;
    ...
    planar_section(const rhs_type& rhs) :
        verbose(false), isPositive(true), _rhs(rhs), _counter(0), _dist(0) {}
    planar_section(const self_type& other) :
    ...
    */
    double get_yd(double x, double xd) const
    {
        ...
        double d  = sqrt((x + mu) * (x + mu) );
        double r  = sqrt((x - 1 + mu) * (x - 1 + mu) );
        
        double Ustar = (1.0-mu)/d+mu/r + (x*x)/2.0;
        double ydsqr = 2.0*Ustar - _rhs.getJacobi() - xd*xd;
...
class planar_section_reduced : public section<double, N, 2> {
public:
    typedef RHS                              rhs_type;
    typedef typename rhs_type::xstate_type   xstate_type;
    typedef typename rhs_type::state_type    state_type;
    
    static const int state_dim =  N;
    static const int space_dim =  N/2;
    
    typedef section<double, N, 2>                   base_type;
    typedef typename base_type::lvec_type           lvec_type;
    typedef typename base_type::lmat_type           lmat_type;
    typedef typename base_type::lgMatrix            lgMatrix;
    typedef typename base_type::gvec_type           gvec_type;
    typedef typename base_type::lbox_type           lbox_type;
    typedef typename base_type::value_type          value_type;
    typedef planar_section_reduced<rhs_type, N>     self_type;
    ...
    planar_section_reduced(const rhs_type& rhs) :
        _rhs(rhs), verbose(false), isPositive(true), _counter(0), _dist(0) {}
    planar_section_reduced(const self_type& other) :
        _rhs(other._rhs), verbose(false), isPositive(other.isPositive), _counter(0), _dist(0) {}
    ...
    /// Tranverse velocity component (perpendicular to section)
    value_type tranverseVelocity( const gvec_type& x) const
    {
        lvec_type sx = project(x).first;
        return get_yd(sx[0],sx[1]);
    }
