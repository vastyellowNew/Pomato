...
int main(int argc, char* argv[])
{
    ...
    //Storage assuming there is one winding number per trajectory (not true in CR3BP model)
    ...
    #pragma omp parallel
    {
        ...
        for (int n = 0 ; n < nsamples ; ++n) {
            ...
            th_orbits[th_id].back().first = xavier::best_period(vec, maxi/4, m);//**This may not work
