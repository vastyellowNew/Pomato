...
#include <cr3bp/ThetaSection.hpp>
...
typedef orbital::ThetaSection<rhs_type, 6, 42>                              section_type;
...
void printUsageAndExit(const std::string& what)
{
    ...
    std::cerr
            << "USAGE: " << me << " [options]\n"
            << "DESCRIPTION: Compute Map for OC for Angular Section in restricted 3-body problem\n"
            << "OPTIONS:\n"
            ...
            << " -a  | --angle <float>            Angle to define section (deg from x-axis)\n"
...
int main(int argc, char* argv[])
{
    ...
    bounds.max()[0] = 1.5;
    bounds.max()[1] = 2.5;
    theta = 1.05778417839148; //rads (~60.6deg) L4 by defualt
    ...
    for (int i=1 ; i<argc ; ++i) {
        ...
        if (arg == "-h") {
            ...
            if (i == argc-1) {
                printUsageAndExit("missing angle");
            }
            theta = ((double) atof(argv[++i]))*M_PI / 180.0;
    ...
    #pragma omp parallel
    {
        ...
        for (int n=0 ; n<nseeds ; ++n) {
            ...
            try {
                ...
                // Or insufficient amount of returns to the provided region
