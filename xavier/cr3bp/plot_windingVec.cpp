...
int main(int argc, char* argv[])
{
    ...
    #pragma omp parallel
    {
        ...
        for (int n=0 ; n<nseeds ; ++n) {
            ...
            //double w = 2.*M_PI*orbit.size()/orbit.back().delta_theta;
            //if (std::isnan(w) || std::isinf(w)) w = 1000;
            
            double wxxdot = orbit.back().delta_theta[0]/(2.*M_PI*orbit.size());
            double wxydot = orbit.back().delta_theta[1]/(2.*M_PI*orbit.size());
            double wxdotydot = orbit.back().delta_theta[2]/(2.*M_PI*orbit.size());
            if (std::isnan(wxxdot) || std::isinf(wxxdot)) {
                wxxdot = 1000;
            }
            if (std::isnan(wxydot) || std::isinf(wxydot)) {
                wxydot = 1000;
            }
            if (std::isnan(wxdotydot) || std::isinf(wxdotydot)) {
                wxdotydot = 1000;
            }
...
#pragma openmp atomic
                raster[m] = wxxdot;//How do I set multiple?
            }
